...
export default function ProductCatalog({
  ...
  return (
    <div className="w-full lg:w-7/12 h-1/2 lg:h-full bg-white overflow-hidden flex flex-col shadow-sm">
          ...
          <div className="flex space-x-2">
            ...
            <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="px-3 py-1.5 h-9">
      ...
      {/* Product Grid */}
      <div className="flex-1 p-4 overflow-y-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {isLoading ? (
          // Loading state
          ...
          // Empty state
          <div className="col-span-4 flex justify-center items-center h-40 text-slate-400">
            No products found
          </div>
        ) : (
          // Product cards
          products.map((product) => (
            <div
              key={product.id}
              className="bg-white border border-slate-200 rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden cursor-pointer"
